...
class M_Pengguna extends SENE_Model{
  ...
  public function getByEmail($email){
    $sql="SELECT `id`,`email`,`password`,`name_first`,`name_last`,`class`,`active`,`confirmed`, `confirm_code` FROM `t_pengguna` WHERE `email` = ".$this->db->esc($email)." ";
		// die($sql);
    return $this->select($sql);
  }
 ...
 public function resetpassword($code,$password){
    $sql="UPDATE `t_pengguna` SET `password` = PASSWORD(".$this->db->esc($password).") WHERE `confirm_code` = ".$this->db->esc($code)."";
    // die($sql); 
    return $this->exec($sql);
