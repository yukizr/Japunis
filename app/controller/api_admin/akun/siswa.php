...
class Siswa extends JI_Controller{
  ...
  public function index(){
    $d = $this->__init();
		...
		$dcount = $this->clum->countAll($keyword,$utype="siswa");
		$ddata = $this->clum->getAll($page,$pagesize,$sortCol,$sortDir,$keyword,$utype="siswa");
	...
	public function edit($id){
		...
		if($id>0 && strlen($di['email'])>1){
			$learn_user = $this->clum->getJoinById($id);
			$b_user_id = 0;
			if(isset($learn_user->b_user_id)){
				$b_user_id = $learn_user->b_user_id;
				$check = $this->bum->checkEmail($di['email'],$b_user_id); //1 = sudah digunakan
				if(empty($check)){
					$res = $this->bum->update($b_user_id,$di);
					$res2 = $this->clum->updateByUserId($b_user_id,$du);
					if($res && $res2){
						$this->status = 100;
				...
				$this->status = 104;
				$this->message = 'Cant find learn_user';
			}
		}else{
			$this->status = 445;
			$this->message = 'One or more paramater is invalid or missing';
		}
		$this->__json_out($data);
	}
	public function hapus($id){
		...
		$learn_user = $this->clum->getJoinById($id);
		if(isset($learn_user->id)){
			$b_user_id = $learn_user->id;
			$this->status = 100;
			$this->message = 'Berhasil';
			$res = $this->clum->delByUserId($b_user_id);
			$res2 = $this->bum->del($b_user_id);
			if(!$res || !$res2){
				$this->status = 902;
	...
	public function edit_password($id){
		...
		if($id>0 && strlen($du['password'])>4){
			...
			$this->status = 445;
			$this->message = 'One or more parameter are required or missing';
		}
		$this->__json_out($data);
	}
}
