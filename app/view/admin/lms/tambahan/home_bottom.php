...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		...
		//$("#modal-preloader").modal("show");
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?php  echo base_url("api_admin/lms/tambahan/"); ?>",
			...
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				...
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						window.location = '<?php echo base_url_admin('lms/tambahan/edit/'); ?>'+id;
					});
					fnCallback(response);
...
$("#fedit").on("submit",function(e){
	e.preventDefault();
	var fd = new FormData($(this)[0]);
	var url = '<?php echo base_url("api_admin/lms/tambahan/edit/"); ?>';
	$.ajax({
		type: $(this).attr('method'),
		url: url,
		data: fd,
...
$("#bhapus").on("click",function(e){
	...
	if(id){
		...
		if(c){
			var url = '<?php echo base_url('api_admin/lms/tambahan/hapus/'); ?>'+id;
...
$("#aedit").on("click",function(e){
	...
	setTimeout(function(){
		...
		window.location = '<?php echo base_url_admin('lms/tambahan/edit/'); ?>'+ieid;
	},333);
});
