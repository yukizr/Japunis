...
$("#fedit").on("submit",function(e){
	...
	var url = '<?php echo base_url("api_admin/lms/abjad/edit/".$abjad_id); ?>';
	$.ajax({
		...
		success: function(respon){
			if(respon.status=="100" || respon.status == 100){
				...
				setTimeout(function(){
					window.location = '<?php echo base_url_admin('lms/abjad/'); ?>';
				},3000);
			}else{
...
$("#bhapus").on("click",function(e){
	...
	var c = confirm('Apakah anda yakin?');
	if(c){
		$.get('<?php echo base_url('api_admin/lms/abjad/hapus/'.$abjad_id); ?>').done(function(dt){
			if(dt.status=='100' || dt.status == 100){
