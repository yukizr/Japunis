...
$("#fedit").on("submit",function(e){
	...
	var url = '<?php echo base_url("api_admin/lms/pelajaran/edit/".$pelajaran_id); ?>';
	$.ajax({
		...
		success: function(respon){
			if(respon.status=="100" || respon.status == 100){
				...
				setTimeout(function(){
					window.location = '<?php echo base_url_admin('lms/pelajaran/'); ?>';
				},3000);
			}else{
...
$("#bhapus").on("click",function(e){
	...
	var c = confirm('Apakah anda yakin?');
	if(c){
		$.get('<?php echo base_url('api_admin/lms/pelajaran/hapus/'.$pelajaran_id); ?>').done(function(dt){
			if(dt.status=='100' || dt.status == 100){
				gritter('<h4>Berhasil</h4><p>Penghapusan data telah berhasil!</p>','success');
				setTimeout(function(){
					window.location = '<?php echo base_url_admin('lms/pelajaran/'); ?>';
				},3000);
			}else{
