...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		...
		//$("#modal-preloader").modal("show");
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?php  echo base_url("api_admin/lms/pelajaran/"); ?>",
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				...
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						window.location = '<?php echo base_url_admin('lms/quiz/tambah/'); ?>'+id;
					});
					fnCallback(response);
...
$("#apilih_pelajaran_id").on("click",function(e){
	e.preventDefault();
	var v = $("#is_pelajaran_id").val();
	if(v.length>0){
		window.location='<?php echo base_url_admin('lms/quiz/tambah/'); ?>'+v;
	}
});
...
$("#ftambah").on("submit",function(e){
	...
	var url = '<?php echo base_url("api_admin/lms/pelajaran/tambah/"); ?>';
	$.ajax({
...
$("#fedit").on("submit",function(e){
	e.preventDefault();
	var fd = new FormData($(this)[0]);
	var url = '<?php echo base_url("api_admin/lms/pelajaran/edit_form/"); ?>';
	$.ajax({
		type: $(this).attr('method'),
		url: url,
		data: fd,
...
$("#bhapus").on("click",function(e){
	...
	if(id){
		...
		if(c){
			var url = '<?php echo base_url('api_admin/lms/pelajaran/hapus/'); ?>'+id;
...
$("#aedit").on("click",function(e){
	...
	setTimeout(function(){
		...
		window.location = '<?php echo base_url_admin('lms/pelajaran/edit/'); ?>'+ieid;
	},333);
});
