...
$("#fedit").on("submit",function(e){
  ...
  var url = '<?php echo base_url('api_admin/lms/quiz/edit/'.$elqm->id); ?>';
  $.ajax({
    type: 'POST',
    url: url,
    data: fd,
    ...
    success: function(respon){
      if(respon.status=="100" || respon.status == 100){
        ...
        setTimeout(function(){
          window.location = '<?php echo base_url_admin('lms/quiz/'); ?>';
        },3000);
      }else{
...
$("#bhapus").on("click",function(e){
  ...
  if(ieid){
    var c = confirm('apakah anda yakin?');
    if(c){
      var url = '<?php echo base_url('api_admin/lms/quiz/hapus/'.$elqm->id.'/'); ?>'+ieid;
