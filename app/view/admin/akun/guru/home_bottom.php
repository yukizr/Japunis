...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		...
		//$("#modal-preloader").modal("show");
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?php  echo base_url("api_admin/akun/guru/"); ?>",
...
$("#fedit_password").on("submit",function(e){
	...
	var url = '<?php echo base_url("api_admin/akun/guru/edit_password/"); ?>';
	$.ajax({
...
$("#ftambah").on("submit",function(e){
	e.preventDefault();
	var fd = new FormData($(this)[0]);
	var url = '<?php echo base_url("api_admin/akun/guru/tambah/"); ?>';
	$.ajax({
		type: $(this).attr('method'),
		url: url,
		data: fd,
...
$("#modal_edit").on("shown.bs.modal",function(e){
	var url = '<?php echo base_url(); ?>api_admin/akun/guru/detail/'+ieid;
...
$("#fedit").on("submit",function(e){
	e.preventDefault();
	var fd = new FormData($(this)[0]);
	var url = '<?php echo base_url("api_admin/akun/guru/edit/"); ?>';
	$.ajax({
		type: $(this).attr('method'),
		url: url,
		data: fd,
...
$("#bhapus").on("click",function(e){
	...
	if(id){
		...
		if(c){
			var url = '<?php echo base_url('api_admin/akun/guru/hapus/'); ?>'+id;
			$.get(url).done(function(response){
				$("#modal_edit").modal("hide");
